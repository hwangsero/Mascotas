...
<script>
	...
	$(document).ready(function() {
		$('#write').click(function() {
			location.href="<%= request.getContextPath()%>/menu/com_Review/com_ReviewReg.do";
		});
	});
	
	function doAction(no) {
		<c:if test="${ not empty userVO }">
			location.href="<%= request.getContextPath() %>/menu/com_Review/com_ReviewDetail.do?no="+no+"&type=list";
		</c:if>
...
<hr>
	...
	<br><br>
	 <c:forEach begin="1" end="${pageNum}" var="page"><a href="<%=request.getContextPath()%>/menu/com_Review/com_ReviewList.do?pageNo=${page}">${page}</a> </c:forEach><br>
   <form method="post" action="<%=request.getContextPath() %>/menu/boardSearch.do">
   <input type="hidden" name="cate" value="후기게시판">
   <input type="hidden" name="url" value="/menu/com_Review/com_ReviewList.jsp?pageNo=1">
